...
| 02 | EC2 stop → 자동 재시작 | EventBridge(EC2 state-change) | Lambda start | ✅ live `cases/02-ec2-restart/` |
| 03 | Config rule + SSM remediation | restricted-ssh 등 | AWS-Disable... | ✅ live `cases/03-config-remediation/` |
| 04 | 태그 위반 탐지 → 알림 | EventBridge/Config | SNS | ✅ live `cases/04-tag-violation/` |
