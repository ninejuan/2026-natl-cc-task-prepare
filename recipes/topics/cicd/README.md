...
| # | 케이스 | 흐름 | 검증 |
|---|---|---|---|
| 01 | GHA → ECR → ECS 롤링 | build→push→register-taskdef→update-service | ✅ **live E2E** `cases/01-gha-ecs/`(실제 repo 로 OIDC→ECR→ECS `:1`→`:2`, `curl` 로 새 앱 응답 확인) |
...
| 03 | GHA OIDC 인증(키 없이) | assume-role-with-web-identity | ✅ **live E2E**(실제 GHA 에서 assume 성공). ★`sub` 불변ID 함정 규명 — `cases/03-oidc/` |
