...
| # | 케이스 | 방식 | 기반 |
|---|---|---|---|
| 01 | SAML → IAM 2 Role (2024 원형) | IAM SAML provider + role×2 | cncf/keycloak + iam-federation ✓ |
| 02 | OIDC → IAM Role | OIDC provider | iam-federation |
| 03 | IdC + 외부 IdP(Keycloak) | Identity Center | iam-federation 케이스 C |
| 04 | 그룹별 Role 매핑 | Keycloak group → SAML attr | `cases/04-group-mapping/` |
